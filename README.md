# Android Tablet Resolution Fix
This magisk module fixes the Android UI on the Android Tablet and allows your device to render games and apps at higher resolution. I have used Xiaomi Pad 5 for this and its working absolutely great.
# How it works?
Some unknown or unfamilier bands tablets are unnoticed by the developers. Developers optimize the some of the big band tablets. They develop the apps and games with "Brand" on mind too . Similarly here Samsung Tablets are like this. Samsung Tablets are actually rendered and treated as a Android Tablet where some other brands tablets are treated as Android. So we are spoofing our tablet as Samsung Tablet (S8+) to make this working. 
# How to install / uninstall
You must rooted with KSU / Magisk to make this working.
You can uninstall by clicking remove
You can simply flash this file in Magisk / KSU
# Side Effects 
As this module is to increase the resolution this will result in High GPU usage in games which may lead to face fps drops  , Increase in temperature ...
    

